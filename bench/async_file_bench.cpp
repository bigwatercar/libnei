...
BenchEntry BenchWrite(nei::AsyncFile &file,
  ...
  file.OpenAsync(path_str,
                 nei::AsyncFile::OpenMode::kWriteOnly,
  ...
  for (std::size_t i = 0; i < nchunks; ++i) {
    ...
    file.WriteAsync(b, sz, off, [&](bool, std::size_t, nei::AsyncFile::Error) {
  ...
  e.elapsed_us = static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count());
  e.throughput_mbps =
      static_cast<double>(total_bytes) / (1024.0 * 1024.0) / (static_cast<double>(e.elapsed_us) / 1'000'000.0);
  return e;
}
...
BenchEntry BenchRead(nei::AsyncFile &file,
  ...
  std::atomic<bool> ok{false};
  file.OpenAsync(path_str,
                 nei::AsyncFile::OpenMode::kReadOnly,
                 nei::AsyncFile::OpenDisposition::kOpenExisting,
                 bg,
                 [&](bool s, nei::AsyncFile::Error e) {
                   ok.store(s && e.ok(), std::memory_order_release);
                   ev.Signal();
  ...
  for (std::size_t i = 0; i < nchunks; ++i) {
    ...
    const std::size_t sz = (std::min)(chunk_size, total_bytes - off);
    auto b = nei::scoped_refptr<nei::IOBuffer>(new nei::IOBufferWithSize(sz));
    file.ReadAsync(b, sz, off, [&](bool, std::size_t, nei::AsyncFile::Error) {
      in_flight.fetch_sub(1, std::memory_order_acq_rel);
      if (done.fetch_add(1, std::memory_order_acq_rel) + 1 == nchunks)
        all.Signal();
    });
...
void PrintHeader() {
  std::cout << std::left << std::setw(18) << "Operation" << std::setw(12) << "Chunk" << std::setw(14) << "Total"
            << std::setw(12) << "Time(ms)" << std::setw(14) << "Throughput" << std::endl;
  std::cout << std::string(70, '-') << std::endl;
}

void PrintRow(const BenchEntry &e) {
  std::cout << std::left << std::setw(18) << e.label << std::setw(12) << FormatSize(e.chunk_size) << std::setw(14)
            << FormatSize(e.total_bytes) << std::setw(12) << std::fixed << std::setprecision(2)
            << (static_cast<double>(e.elapsed_us) / 1000.0) << std::setw(14) << std::fixed << std::setprecision(1)
...
int main() {
  ...
  const std::vector<std::size_t> kChunks = {64 * 1024, 256 * 1024, 1 * 1024 * 1024, 4 * 1024 * 1024};
#endif
  ...
  for (auto cs : kChunks) {
    ...
    auto p = base.parent_path() / (base.stem().string() + "_w_" + std::to_string(cs) + ".bin");
  ...
  const auto rpath = base.parent_path() / (base.stem().string() + "_r_src.bin");
  {
  ...
  for (const auto &e : results) {
    if (e.label == std::string("sequential-write") && e.throughput_mbps > bw) {
      bw = e.throughput_mbps;
      cw = e.chunk_size;
    }
    if (e.label == std::string("sequential-read") && e.throughput_mbps > br) {
      br = e.throughput_mbps;
      cr = e.chunk_size;
    }
  }
  std::cout << "Best write: " << std::fixed << std::setprecision(1) << bw << " MB/s at " << FormatSize(cw) << " chunk"
            << std::endl;
