...
struct ReadExactState : public std::enable_shared_from_this<ReadExactState> {
  ...
  void Issue() {
    auto slice = nei::scoped_refptr<nei::IOBuffer>(
        new nei::WrappedIOBuffer(holder.buf->data() + offset));
    stream->ReadAsync(
        slice, total - offset,
        [self = shared_from_this(), slice](bool ok, std::size_t n) {
          if (!ok || n == 0) {
...
void ReadExact(const std::shared_ptr<nei::PipeInputStream>& stream,
               std::size_t bytes,
               ReadExactCallback callback) {
...
struct WriteExactState : public std::enable_shared_from_this<WriteExactState> {
  ...
  void Issue() {
    auto slice = nei::scoped_refptr<nei::IOBuffer>(
        new nei::WrappedIOBuffer(base->data() + offset));
    stream->WriteAsync(
        slice, total - offset,
        [self = shared_from_this(), slice](bool ok, std::size_t n) {
          if (!ok || n == 0) {
...
struct ChildLoopState : public std::enable_shared_from_this<ChildLoopState> {
  ...
  void RunNext() {
    ...
    ReadExact(input, payload_size,
              [self = shared_from_this()](bool read_ok, BufferHolder holder) {
                if (!read_ok) {
                  self->done->Signal();
                  return;
                }
                WriteExact(self->output, holder.sized, self->payload_size,
                           [self](bool write_ok) {
                             if (!write_ok) {
                               self->done->Signal();
                               return;
...
bool RunChild(const std::string& read_pipe_name,
  ...
  io_runner->PostTask(FROM_HERE, [io_runner, &done, &ok, read_handle,
                                  write_handle, payload_size, iterations]() {
  ...
  void Kick() {
    ...
    ReadExact(input, payload_size,
              [self = shared_from_this()](bool read_ok, BufferHolder holder) {
                ...
                self->latencies_us.push_back(
                    std::chrono::duration_cast<Microseconds>(now - self->start_time)
                        .count());
                ++self->current;
                self->Kick();
              });
    ...
    WriteExact(output, payload, payload_size,
               [self = shared_from_this()](bool write_ok) {
                 if (!write_ok) {
                   self->done->Signal();
                 }
               });
...
bool RunParent(std::size_t payload_size, int iterations, Stats* stats_out) {
  ...
  io_runner->PostTask(FROM_HERE, [io_runner, &done, &ok, state, write_server,
                                  read_server, payload_size, iterations]() {
    auto input = std::make_shared<nei::PipeInputStream>(io_runner);
    auto output = std::make_shared<nei::PipeOutputStream>(io_runner);
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 6 && std::string(argv[1]) == "--child") {
    return RunChild(argv[2], argv[3], static_cast<std::size_t>(std::atoll(argv[4])),
                    std::atoi(argv[5]))
               ? 0
               : 1;
  }
  ...
  std::cout << "=== PipeStream Cross-Process Ping-Pong Benchmark (Windows) ==="
            << std::endl;
