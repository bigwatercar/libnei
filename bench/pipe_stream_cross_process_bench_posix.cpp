...
struct BenchLoop : public std::enable_shared_from_this<BenchLoop> {
  ...
  void Kick() {
    ...
    input->ReadAsync(read_holder.buf, payload_size, [self = shared_from_this()](bool read_ok, std::size_t n) {
      if (!read_ok || n != self->payload_size) {
        self->done->Signal();
        return;
      }
      self->latencies->push_back(std::chrono::duration_cast<Microseconds>(Clock::now() - self->start_time).count());
      ++self->current;
      self->Kick();
    });
    output->WriteAsync(payload_view, payload_size, [self = shared_from_this()](bool write_ok, std::size_t n) {
...
bool RunParent(std::size_t payload_size, int iterations, Stats *stats_out) {
  ...
  if (pid == 0) {
    close(parent_to_child[1]);
    close(child_to_parent[0]);
    const bool ok = RunChild(parent_to_child[0], child_to_parent[1], payload_size, iterations);
    _exit(ok ? 0 : 1);
  }
  ...
  const bool finished = done.TimedWait(std::chrono::seconds(60)) && ok.load(std::memory_order_acquire);
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "=== PipeStream Cross-Process Ping-Pong Benchmark (POSIX) ===" << std::endl;
