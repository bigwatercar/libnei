...
int main() {
  ...
  const bool block_finished = block_done_future.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready;
  ...
  std::cout << "shutdown behavior: " << ((block_finished && skip_was_skipped && elapsed_is_expected) ? "PASS" : "FAIL")
            << '\n';
