...
int main() {
  ...
  const auto user_blocking_status_before_release = user_blocking_done_future.wait_for(std::chrono::milliseconds(20));
  const bool user_blocking_waited_for_normal_group = user_blocking_status_before_release == std::future_status::timeout;
  ...
  std::cout << "best effort isolated group: " << (best_effort_ran_while_normal_busy ? "PASS" : "FAIL") << '\n';
  std::cout << "normal group stayed busy: " << (user_blocking_waited_for_normal_group ? "PASS" : "FAIL") << '\n';
