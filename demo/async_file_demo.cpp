...
bool RunDemo(nei::AsyncFile& file,
  ...
  nei::WaitableEvent open_done(nei::WaitableEvent::ResetPolicy::kAutomatic,
                               false);
  ...
  if (!open_ok.load(std::memory_order_acquire)) {
    std::cerr << "[demo] OpenAsync failed, error="
  ...
  // --- Write ---
  nei::WaitableEvent write_done(nei::WaitableEvent::ResetPolicy::kAutomatic,
                                false);
  ...
  // --- Read back ---
  nei::WaitableEvent read_done(nei::WaitableEvent::ResetPolicy::kAutomatic,
                               false);
