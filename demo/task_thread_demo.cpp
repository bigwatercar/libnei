...
int main() {
  ...
  auto make_task = [&](const char *name,
    ...
    return nei::BindOnce(
        [&](const char *inner_name,
          ...
          std::cout << inner_name << " task " << inner_index << " running on thread " << std::this_thread::get_id()
                    << '\n';
  ...
  const bool same_sequence_is_serial = !serial_violation_a.load() && !serial_violation_b.load();
  ...
  std::cout << "cross sequence parallel: " << (cross_sequence_is_parallel ? "PASS" : "FAIL") << '\n';
