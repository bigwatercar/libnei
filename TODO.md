...
config = nei_log_get_config(config_id);  // 获得读锁 → 查表 → 释放读锁
                                         // ← 返回指向 s_custom_configs[slot] 的指针
if (config != NULL) {
  _nei_log_emit_message(config, ...);    // ← 在无锁状态下使用指针
}

// 同时生产线程 - nei_log_add_config(config_id, ...)
...
**竞态窗口**（极端时序）：
```
时间 t1: 消费 - config = nei_log_get_config("logger1")  // 获得指向 s_custom_configs[1]
时间 t2: 消费 - 释放读锁
时间 t3: 消费 - config->level_flags 读完毕
  ↓（潜在延迟，如系统调度）
时间 t4: 生产 - nei_log_add_config("logger1", new_cfg)  // 覆盖 s_custom_configs[1]
...
```c
// 调用链路
nei_log_get_config(id)
  → _nei_log_config_lock_read()  // 获得读锁
  → _nei_log_find_slot_by_id(id)
    → _nei_log_ensure_config_table_initialized()

      // ❌ 这个函数在 *读锁* 下执行：
      if (!s_config_table_initialized) {
        memset(s_config_id_index, -1, ...);      // 写全局共享结构！
        s_config_table_initialized = 1;           // 写标志位！
      }
```
...
- RWLock 的"读锁"保护"不可变读"
- 这里进行了"初始化级写"
- 其他读锁线程同时读 `s_config_id_index` → **数据竞争**
...
nei_log_get_config      nei_log_get_config
  → acquire_read()
  → ensure_init()
    → memset(table)     → _find_slot()
                          → 读未初始化的 table（竞争！）
    → initialized = 1
```

**当前状态**：✅ 如果首次调用在单线程上下文成功，后续多线程调用可能无症状（因为已初始化）；但首次多线程调用有竞争
...
```c
// 消费线程对每条记录
config = nei_log_get_config(config_id);  // ← 申请 RWLock 读锁
```
...
```c
// 配置表指针
static int8_t *s_config_id_index_ptr = &s_config_id_index_primary[0];

// 更新时
void nei_log_add_config(...) {
  // 1. 复制整表
  int8_t *new_table = malloc(...);
  memcpy(new_table, s_config_id_index_ptr, ...);

  // 2. 修改副本
  _rebuild_in(new_table, ...);

  // 3. 原子交换
  int8_t *old = atomic_exchange(&s_config_id_index_ptr, new_table);
...
| Config 查询 | L345-352 | `nei_log_get_config()` |
| Config 更新 | L356-396 | `nei_log_add_config()` |
| Config 初始化 | L685-706 | `_nei_log_ensure_config_table_initialized()` |
