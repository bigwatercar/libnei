...
bool FilePathWatcher::Impl::Watch(const std::string &path, bool recursive, Callback callback) {
  ...
  if (pump) {
    controller_.StartWatching(
        pump, static_cast<NativeIOHandle>(inotify_fd_), MessagePumpForIO::FdWatchController::Mode::READ, this);
    watching_ = true;
    return true;
  }
  ...
  while (watching_ && inotify_fd_ >= 0) {
    ...
    if (n > 0) {
      for (const char *ptr = inotify_buf_; ptr < inotify_buf_ + n;) {
        ...
        std::string dir_path = (it != wd_to_path_.end()) ? it->second : watch_root_;
        ...
        if (relative.size() > watch_root_.size() && relative.compare(0, watch_root_.size(), watch_root_) == 0) {
          relative = relative.substr(watch_root_.size() + (watch_root_ == "/" ? 0 : 1));
        }
        ...
        if (recursive_ && (event->mask & (IN_CREATE | IN_MOVED_TO)) && (event->mask & IN_ISDIR)) {
...
bool FilePathWatcher::Impl::AddWatchRecursive(const std::string &path) {
  std::uint32_t mask = IN_CREATE | IN_DELETE | IN_DELETE_SELF | IN_MODIFY | IN_MOVE_SELF | IN_MOVED_FROM | IN_MOVED_TO
...
FilePathWatcher::ChangeType FilePathWatcher::Impl::MapInotifyMask(std::uint32_t mask) {
