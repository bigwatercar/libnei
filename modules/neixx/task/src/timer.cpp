...
private:
  ...
  void OnTimerFired() {
    ...
    OnceCallback<void()> task = std::move(user_task_);
    // user_task_ 此时为空
...
public:
  ...
  void Stop() {
    ...
    weak_ptr_factory_.InvalidateWeakPtrs();
    user_task_ = RepeatingCallback<void()>();
    is_running_ = false;
  }
...
private:
  ...
  SequenceChecker sequence_checker_;
  Location posted_from_;
  RepeatingCallback<void()> user_task_;
  TimeDelta delay_;
  bool is_running_ = false;
