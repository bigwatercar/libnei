...
void AsyncInputStreamProxy::ReadAsync(DataCallback callback) {
  bool should_arm = false;
  {
    std::lock_guard<std::mutex> lock(state_->lock);
...
// static
void AsyncInputStreamProxy::ArmReadOnIoThread(
...
void AsyncOutputStreamProxy::WriteAsync(std::vector<std::uint8_t> data,
  ...
  runner->PostTask(
      ...
      [state, data = std::move(data), callback = std::move(callback)]() mutable {
        ...
        {
          std::lock_guard<std::mutex> lock(state->lock);
          target = state->close_requested ? nullptr : state->target;
        }
        if (target == nullptr) {
          if (callback) {
            callback(false);
