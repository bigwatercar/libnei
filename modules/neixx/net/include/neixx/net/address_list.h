...
public:
  ...
  IPEndPoint &operator[](size_type idx) noexcept {
    return endpoints_[idx];
  }

  const IPEndPoint &front() const noexcept {
    return endpoints_.front();
  }

  const IPEndPoint &back() const noexcept {
