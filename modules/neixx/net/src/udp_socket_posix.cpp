...
void UDPSocket::Impl::DoSendTo(scoped_refptr<IOBuffer> buf,
  ...
  // ENOBUFS: receive buffer full on loopback — enqueue only.  The write
  //   watcher won't help (send buffer has space); sends are flushed from
  //   DrainRecvQueue tail after reads free receive buffer space.
  ...
  if (errno == ENOBUFS) {
    ...
    // Do NOT arm write watcher — on loopback the send buffer has space;
    // the bottleneck is the receive buffer.  DrainRecvQueue will flush
    // pending sends after reading.
    return;
  }
...
void UDPSocket::Impl::DrainSendQueue() {
  while (true) {
    ...
    // ENOBUFS: receive buffer full on loopback — push back, STOP write
    //   watcher to prevent busy-loop.  On loopback the send buffer is not
    //   the bottleneck; we must wait for RecvFrom to drain the receive
    //   buffer before retrying (see DrainRecvQueue tail).
    if (errno == EAGAIN || errno == EWOULDBLOCK) {
      {
        std::lock_guard<std::mutex> lock(mutex_);
