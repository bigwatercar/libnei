...
bool UDPSocket::Impl::DoBind(const IPEndPoint& local_addr) {
  ...
  DCHECK_MSG(socket_ == INVALID_SOCKET,
             "DoBind: socket already created — duplicate Bind call?");
  ...
  if (bind(socket_, reinterpret_cast<struct sockaddr*>(&sa), sa_len) ==
      SOCKET_ERROR) {
    closesocket(socket_);
...
void UDPSocket::Impl::OnIOCompleted(
    NativeIOHandle /*handle*/, void* overlapped_context,
    std::uint32_t bytes_transferred, std::uint32_t error_code) {
  DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

  auto* ctx = CONTAINING_RECORD(overlapped_context, UdpOverlappedContext,
                                overlapped);
...
bool UDPSocket::Impl::JoinGroup(const IPAddress& group_address) {
  ...
  if (group_address.IsIPv6()) {
    ...
    return setsockopt(socket_, IPPROTO_IPV6, IPV6_JOIN_GROUP,
...
bool UDPSocket::Impl::LeaveGroup(const IPAddress& group_address) {
  ...
  if (group_address.IsIPv6()) {
    ...
    std::memcpy(&mreq6.ipv6mr_multiaddr, group_address.data().data(), 16);
    mreq6.ipv6mr_interface = 0;
    return setsockopt(socket_, IPPROTO_IPV6, IPV6_LEAVE_GROUP,
                      reinterpret_cast<const char*>(&mreq6),
                      sizeof(mreq6)) == 0;
  }
...
bool UDPSocket::Impl::GetLocalAddress(IPEndPoint* out) const {
  ...
  if (getsockname(socket_, reinterpret_cast<struct sockaddr*>(&sa),
                  &sa_len) != 0) {
    return false;
  }
...
IPEndPoint UDPSocket::Impl::SockAddrToIPEndPoint(
    const struct sockaddr_storage& sa, int sa_len) const {
  ...
  if (sa.ss_family == AF_INET6 &&
      sa_len >= static_cast<int>(sizeof(struct sockaddr_in6))) {
