...
std::string HttpResponseWriter::Serialize(const HttpResponse& response) {
    ...
    if (is_chunked) {
        if (!response.body.empty()) {
            result += SerializeChunk(response.body.data(),
                                     response.body.size());
        }
...
std::string HttpResponseWriter::SerializeHeaders(const HttpResponse& response) {
    ...
    for (const auto& h : headers) {
        ...
        if (lower_name == "transfer-encoding") has_transfer_encoding = true;
    }

    // Auto-add Content-Length if body is present and no Transfer-Encoding
    // and no Content-Length already provided.
    if (!response.body.empty() && !has_transfer_encoding && !has_content_length) {
        char buf[32];
...
std::string HttpResponseWriter::SerializeChunk(const void* data,
                                                size_t len) {
