...
bool PipeInputStream::Impl::BindPlatformHandle(PlatformHandle handle) {
  ...
  SetFileCompletionNotificationModes(handle_, FILE_SKIP_COMPLETION_PORT_ON_SUCCESS);

  MessagePumpForIO *pump = MessagePumpForIO::Current();
  if (pump) {
    controller_.StartWatching(
        pump, reinterpret_cast<NativeIOHandle>(handle_), MessagePumpForIO::FdWatchController::Mode::READ, this);
  }
...
void PipeInputStream::Impl::OnIOCompleted(NativeIOHandle /*handle*/,
                                          void *overlapped_context,
                                          std::uint32_t bytes_transferred,
                                          std::uint32_t error_code) {
  if (orphaned_ctx_ && overlapped_context == &orphaned_ctx_->overlapped) {
    orphaned_ctx_.reset();
    if (shutting_down_) {
      delete this;
      return;
  ...
  if (error_code != ERROR_SUCCESS && error_code != ERROR_HANDLE_EOF && error_code != ERROR_BROKEN_PIPE) {
    if (ctx->callback)
      ctx->callback(false, 0u);
    return;
  }
...
void PipeInputStream::Impl::IssueRead(std::size_t buf_len) {
  ...
  DWORD read_bytes = 0;
  const BOOL ok =
      ReadFile(handle_, read_ctx_->buffer->data(), static_cast<DWORD>(buf_len), &read_bytes, &read_ctx_->overlapped);
  if (ok) {
    AsyncInputStream::IOReadCallback cb = std::move(read_ctx_->callback);
    read_ctx_.reset();
    if (read_bytes == 0) {
      ...
      pipe_detail::PostResult(io_task_runner_, std::move(cb), true, static_cast<std::size_t>(read_bytes));
    }
    return;
  }
...
bool PipeOutputStream::Impl::BindPlatformHandle(PlatformHandle handle) {
  ...
  if (pump) {
    controller_.StartWatching(
        pump, reinterpret_cast<NativeIOHandle>(handle_), MessagePumpForIO::FdWatchController::Mode::WRITE, this);
  }
...
void PipeOutputStream::Impl::OnIOCompleted(NativeIOHandle /*handle*/,
                                           void *overlapped_context,
                                           std::uint32_t bytes_transferred,
                                           std::uint32_t error_code) {
  if (orphaned_ctx_ && overlapped_context == &orphaned_ctx_->overlapped) {
...
void PipeOutputStream::Impl::IssueWrite(std::size_t buf_len) {
  ...
  DWORD written = 0;
  const BOOL ok =
      WriteFile(handle_, write_ctx_->buffer->data(), static_cast<DWORD>(buf_len), &written, &write_ctx_->overlapped);
  if (ok) {
    AsyncOutputStream::IOWriteCallback cb = std::move(write_ctx_->callback);
    write_ctx_.reset();
    pipe_detail::PostResult(io_task_runner_, std::move(cb), true, static_cast<std::size_t>(written));
    return;
  }
