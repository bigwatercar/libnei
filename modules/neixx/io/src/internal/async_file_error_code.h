...
inline AsyncFile::Error NormalizeAsyncFileError(std::uint32_t platform_error) {
  ...
  const std::error_condition condition =
      std::error_code(static_cast<int>(platform_error), std::system_category()).default_error_condition();
  out.code = ToAsyncFileErrorCode(condition);
#else
  out.code = ToAsyncFileErrorCode(
      std::error_code(static_cast<int>(platform_error), std::generic_category()).default_error_condition());
#endif
