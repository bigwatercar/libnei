...
public:
  ...
  void Wait() {
#if defined(_WIN32)
    if (SleepConditionVariableCS(&condition_variable_, user_lock_->GetImpl(), INFINITE) == 0) {
      DCHECK(false);
    }
#else
    const int rv = pthread_cond_wait(&condition_variable_, user_lock_->GetImpl());
    DCHECK_EQ(rv, 0);
#endif
  }
  ...
  void TimedWait(std::chrono::milliseconds timeout) {
    ...
    (void)SleepConditionVariableCS(&condition_variable_, user_lock_->GetImpl(), timeout_ms);
#else
    ...
    const int rv = pthread_cond_timedwait(&condition_variable_, user_lock_->GetImpl(), &ts);
