...
std::atomic<CommandLine *> &CurrentProcessCommandLineCache() {
  static std::atomic<CommandLine *> current{nullptr};
  return current;
}

void PublishCurrentProcessCommandLine(std::unique_ptr<CommandLine> command_line) {
  CommandLine *raw = command_line.get();
  CurrentProcessCommandLine() = std::move(command_line);
  CurrentProcessCommandLineCache().store(raw, std::memory_order_release);
}
...
void CommandLine::Init(int argc, const char *const *argv) {
  ...
  PublishCurrentProcessCommandLine(std::make_unique<CommandLine>(argc, argv));
}

#if defined(_WIN32)
void CommandLine::Init() {
  std::lock_guard<std::mutex> lock(CurrentProcessMutex());
  PublishCurrentProcessCommandLine(std::make_unique<CommandLine>());
}
#endif

CommandLine &CommandLine::ForCurrentProcess() {
  if (CommandLine *cached = CurrentProcessCommandLineCache().load(std::memory_order_acquire)) {
    return *cached;
  }
  ...
  CurrentProcessCommandLineCache().store(CurrentProcessCommandLine().get(), std::memory_order_release);
