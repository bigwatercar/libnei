...
static const char *find_last_sep(const char *p, const char *end) {
    const char *sep = NULL;
    while (p < end) {
        if (nei_path_is_separator(*p)) sep = p;
        ++p;
    }
    return sep;
}
...
int nei_path_join(char *dst, size_t size, const char *a, const char *b) {
    ...
    if (size == 0) {
        ...
        if (nei_path_is_absolute(b)) return (int)(blen + 1);
    ...
    /* Full copy. */
    size_t pos = 0;
    memcpy(dst + pos, a, alen); pos += alen;
    if (alen > 0 && !nei_path_is_separator(a[alen - 1])) {
        dst[pos++] = '/';
    }
    memcpy(dst + pos, b, blen); pos += blen;
    dst[pos] = '\0';
    return (int)pos;
}
...
int nei_path_dirname(const char *path, char *buf, size_t size) {
    ...
    if (len == 0) return buf_copy(buf, size, ".", 1);

    const char *end = path + len;
    /* Strip trailing separators, but keep root. */
    while (end > path + 1 && nei_path_is_separator(*(end - 1))) --end;

    const char *sep = find_last_sep(path, end);
    if (sep == NULL) return buf_copy(buf, size, ".", 1);
    ...
    while (sep > path && nei_path_is_separator(*(sep - 1))) --sep;
    if (sep == path) return buf_copy(buf, size, path, 1);
...
int nei_path_basename(const char *path, char *buf, size_t size) {
    ...
    size_t len = strlen(path);
    if (len == 0) return buf_copy(buf, size, "", 0);
...
int nei_path_stem(const char *path, char *buf, size_t size) {
    ...
    if (*start == '.') {
        ...
        while (p < end && *p == '.') ++p;
        const char *dot = NULL;
        if (p < end) dot = memchr(p, '.', (size_t)(end - p));
        if (dot != NULL) end = dot;
    } else {
...
int nei_path_extension(const char *path, char *buf, size_t size) {
    ...
    for (const char *p = end - 1; p > start; --p) {
        if (*p == '.') { last_dot = p; break; }
    }
    if (last_dot == NULL) return buf_copy(buf, size, "", 0);
...
int nei_path_extensions(const char *path, char *buf, size_t size) {
    ...
    if (first_dot == NULL) return buf_copy(buf, size, "", 0);
...
int nei_path_normalize(const char *path, char *buf, size_t size) {
    ...
    while (*p != '\0') {
        ...
        if (out_len > root_len &&
            !nei_path_is_separator(out[out_len - 1])) {
            out[out_len++] = '/';
        }
