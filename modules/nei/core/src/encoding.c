 ...
 * ------------------------------------------------------------------------- */
static int wstr_to_mb_impl(UINT code_page, const wchar_t *src, int src_len, char *buf, size_t size) {
  int needed = WideCharToMultiByte(code_page, 0, src, src_len, NULL, 0, NULL, NULL);
  if (needed <= 0) {
    return -1;
  }
  ...
  if ((size_t)needed >= size) {
    ...
    memset(buf, 0, size);
    WideCharToMultiByte(code_page, 0, src, src_len, buf, (int)(size - 1), NULL, NULL);
    buf[size - 1] = '\0';
    return out_len;
  }
  WideCharToMultiByte(code_page, 0, src, src_len, buf, needed, NULL, NULL);
  buf[needed] = '\0';
  return out_len;
}
...
int nei_wstr_to_utf8(const wchar_t *src, int src_len, char *buf, size_t size) {
...
int nei_mbcs_to_utf8(const char *src, int src_len, char *buf, size_t size) {
  /* Step 1: MBCS -> wchar_t */
...
int nei_utf8_to_mbcs(const char *src, int src_len, char *buf, size_t size) {
