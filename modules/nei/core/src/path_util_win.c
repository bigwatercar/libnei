...
int nei_path_is_executable(const char *path) {
  ...
  return (_stricmp(ext, ".exe") == 0 || _stricmp(ext, ".bat") == 0 || _stricmp(ext, ".cmd") == 0
          || _stricmp(ext, ".com") == 0);
}
...
int nei_path_create_dir(const char *path, int parents) {
  ...
  for (;;) {
    ...
    *p = L'\0';
    ...
    if (tmp[0] != L'\0') {
      char tmp_u8[4096];
      if (nei_wstr_to_utf8(tmp, -1, tmp_u8, 4096) < 0)
        return -1;
      if (!nei_path_is_dir(tmp_u8)) {
        if (_wmkdir(tmp) != 0 && GetLastError() != ERROR_ALREADY_EXISTS)
          return -1;
      }
    }
