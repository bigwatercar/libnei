...
static int nei_random_fill_bytes(void *out, size_t len) {
    ...
    if (fd >= 0) {
        size_t done = 0U;
        while (done < len) {
            const ssize_t n = read(fd, (unsigned char *)out + done,
                                   len - done);
            if (n <= 0) { close(fd); fd = -1; break; }
            done += (size_t)n;
        }
        if (fd >= 0) { close(fd); return NEI_RANDOM_OK; }
    }
#endif

    /* Fallback: xorshift64* PRNG. */
    {
        const uint64_t counter = ++s_degraded_counter;
        uint64_t x = (uint64_t)time(NULL) ^ (uint64_t)clock() ^
                     (uintptr_t)out ^ ((uint64_t)len << 32U) ^ counter;
...
int nei_random_string(char *out, size_t len, const char *charset) {
    ...
    if (out == NULL || len == 0U) return NEI_RANDOM_ERR_INVALID_ARG;

    if (charset == NULL) charset = NEI_RANDOM_DEFAULT_CHARSET;
    charset_len = strlen(charset);
    if (charset_len == 0U) return NEI_RANDOM_ERR_INVALID_ARG;
