...
void nei_crc32_init(nei_crc32_ctx_st *ctx) {
  if (ctx == NULL) {
    return;
  }
...
void nei_crc32_update(nei_crc32_ctx_st *ctx, const void *data, size_t len) {
  ...
  for (i = 0U; i < len; ++i) {
    const uint32_t index = (ctx->state ^ bytes[i]) & 0xFFU;
    ctx->state = nei_crc32_table[index] ^ (ctx->state >> 8U);
  }
}

uint32_t nei_crc32_final(nei_crc32_ctx_st *ctx) {
  if (ctx == NULL) {
    return 0U;
  }
...
uint32_t nei_crc32_sum(const void *data, size_t len) {
  ...
  nei_crc32_init(&ctx);
  nei_crc32_update(&ctx, data, len);
  return nei_crc32_final(&ctx);
}

void nei_crc32_to_hex(uint32_t checksum, char out_hex[NEI_CRC32_HEX_SIZE]) {
  ...
  for (i = 0U; i < 8U; ++i) {
    const uint32_t shift = (uint32_t)((7U - i) * 4U);
    out_hex[i] = hex[(checksum >> shift) & 0x0FU];
  }
  out_hex[8] = '\0';
}

int nei_crc32_sum_hex(const void *data, size_t len, char out_hex[NEI_CRC32_HEX_SIZE]) {
  if (out_hex == NULL || (data == NULL && len > 0U)) {
    return -1;
  }
  nei_crc32_to_hex(nei_crc32_sum(data, len), out_hex);
  return 0;
}

int nei_crc32_file_sum(const char *file_path, uint32_t *out_checksum) {
  FILE *fp;
  uint8_t buf[4096];
  nei_crc32_ctx_st ctx;
  size_t n;

  if (file_path == NULL || out_checksum == NULL) {
    return -1;
  }
  ...
  fclose(fp);
  *out_checksum = nei_crc32_final(&ctx);
  return 0;
}

int nei_crc32_file_sum_hex(const char *file_path, char out_hex[NEI_CRC32_HEX_SIZE]) {
  uint32_t checksum;

  if (out_hex == NULL) {
    return -1;
  }
  if (nei_crc32_file_sum(file_path, &checksum) != 0) {
    return -1;
  }
  nei_crc32_to_hex(checksum, out_hex);
  return 0;
}
