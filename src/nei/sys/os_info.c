...
#pragma warning(pop)
...
#elif defined(__APPLE__)
  {
    ...
    if (sysctlbyname("kern.osproductversion", version, &len, NULL, 0) == 0 && len > 0 && len <= sizeof(version)) {
...
int nei_get_kernel_version(char *buf, size_t size) {
...
#ifdef _WIN32
  {
    ...
    char result[64];
    int len = snprintf(result, sizeof(result), "%lu.%lu.%lu", vi.dwMajorVersion, vi.dwMinorVersion, vi.dwBuildNumber);
    if (len < 0 || (size_t)len >= sizeof(result)) {
      return -1;
    }
