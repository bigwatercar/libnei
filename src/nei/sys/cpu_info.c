...
int nei_get_cpu_physical_count(void) {
  ...
  SYSTEM_LOGICAL_PROCESSOR_INFORMATION *info = (SYSTEM_LOGICAL_PROCESSOR_INFORMATION *)malloc(buf_size);
  if (info == NULL) {
    return nei_get_cpu_count();
  }
...
int nei_get_cpu_arch(char *buf, size_t size) {
...
#else
  {
    ...
    if (strcmp(machine, "x86_64") == 0 || strcmp(machine, "amd64") == 0) {
      ...
      arch = "aarch64";
    } else if (strcmp(machine, "i686") == 0 || strcmp(machine, "i386") == 0 || strcmp(machine, "i586") == 0) {
      arch = "x86";
