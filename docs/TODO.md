...
## 全库后续路线 TODO（2026-04-26）

> 说明：本节用于记录 Task / Strings / IO / Process 四大模块的下一阶段计划；下文保留日志子系统专项问题清单。

### A. 稳定性与回归基线（P0-P1）

- [ ] 建立长稳测试矩阵（12h/24h）：高并发 Task、海量异步 IO、进程频繁拉起/回收
- [ ] 增加故障注入场景：句柄耗尽、子进程异常退出、管道背压、线程池饥饿
- [ ] 固化性能门槛（CI 基线）：吞吐、P99 延迟、内存占用、上下文切换次数
- [ ] 补齐跨平台回归清单：Windows(IOCP) / Linux(epoll+signalfd) 结果对齐

### B. 可观测性（P1）

- [ ] 打通统一 tracing 事件：Task 入队/执行、IO 提交/完成、Process 启动/退出
- [ ] 增加运行时统计导出：队列深度、排队时延、IO 吞吐、退出码分布
- [ ] 提供问题诊断快照能力（最小开销默认关闭，按需开启）

### C. 进程与 IPC 平台能力（P1-P2）

- [ ] 在 Process 之上增加进程编排语义：依赖启动、超时治理、优雅停机
- [ ] 设计统一 IPC 抽象（命名管道/UDS/共享内存）与消息协议边界
- [ ] 增加大对象传输路径评估：零拷贝与背压协商策略

### D. 产品化与工程化（P1-P2）

- [ ] 明确 ABI/API 稳定边界和版本策略
- [ ] 增加开发者接入文档：Task/IO/Process 最佳实践与反模式
- [ ] 完善 CI 质量门禁：单测、集成测、性能回归、静态分析、平台矩阵

### E. 90 天里程碑（草案）

- [ ] M1（1-3 周）：长稳测试与故障注入框架上线，首版性能基线落地
- [ ] M2（4-6 周）：统一 tracing 与关键统计打通，核心链路可观测
- [ ] M3（7-9 周）：进程编排最小闭环 + IPC 抽象初版
- [ ] M4（10-12 周）：发布门禁完善（ABI/API 策略 + CI 全链路）

**文档版本**：v2.3 | **日期**：2026-04-26
...
|------|--------|--------|------|------|
| [x] #8 文档 | P0 | 1-2h | — | ✅ 已完成 |
| [x] #8 消费线程检测（运行时消费线程标识） | P0 | 2-4h | — | ✅ 已完成 |
