...
std::filesystem::path MakeTempFilePath(const char *name_hint) {
  ...
  p /= std::string("nei_file_stream_adapter_") + name_hint + "_" + std::to_string(static_cast<long long>(ticks))
       + ".txt";
  return p;
}
...
TEST(FileInputStreamAdapterTest, BridgesAsyncFileAndAsyncLineReader) {
  ...
  writer_file->WriteAsync(write_buf, bytes.size(), 0, [&](bool success, std::size_t wrote, AsyncFile::Error error) {
  ...
  writer_file->CloseAsync([&]() { write_close_barrier.Signal(); });
