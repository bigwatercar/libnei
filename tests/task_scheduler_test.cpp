...
TEST(TaskSchedulerTest, DifferentSequencesCanRunInParallel) {
  ...
  seq_a->PostTask(
      FROM_HERE,
      nei::BindOnce(blocking_task, std::ref(entered), std::ref(both_started), release_future, std::ref(done_a)));
  seq_b->PostTask(
      FROM_HERE,
      nei::BindOnce(blocking_task, std::ref(entered), std::ref(both_started), release_future, std::ref(done_b)));

  EXPECT_EQ(both_started_future.wait_for(std::chrono::milliseconds(300)), std::future_status::ready);
...
TEST(TaskSchedulerTest, ShutdownKeepsBlockTasksAndSkipsSkipTasks) {
...

  {
    ...
    thread_pool.PostDelayedTaskWithTraits(
        ...
        nei::BindOnce([](std::atomic<bool> &skip_ran_inner) { skip_ran_inner.store(true, std::memory_order_release); },
                      std::ref(skip_ran)),
...
TEST(TaskSchedulerTest, PostingSkipTaskAfterShutdownIsIgnored) {
  ...
  thread_pool.PostTaskWithTraits(
      FROM_HERE,
      nei::TaskTraits::UserVisible().WithShutdownBehavior(nei::ShutdownBehavior::SKIP_ON_SHUTDOWN),
      nei::BindOnce([](std::promise<void> &done_inner) { done_inner.set_value(); }, std::ref(should_not_run)));

  EXPECT_EQ(should_not_run_future.wait_for(std::chrono::milliseconds(120)), std::future_status::timeout);
}
...
TEST(TaskSchedulerTest, BestEffortCompensationCanBeEnabledViaOptions) {
  ...
  thread_pool.PostTaskWithTraits(
      ...
      nei::BindOnce(
          [](std::promise<void> &started, std::shared_future<void> release, std::promise<void> &done) {
            started.set_value();
            release.wait();
            done.set_value();
          },
  ...
  ASSERT_EQ(blocker_started_future.wait_for(std::chrono::milliseconds(300)), std::future_status::ready);
  ...
  EXPECT_EQ(quick_done_future.wait_for(std::chrono::milliseconds(300)), std::future_status::ready);
...
TEST(TaskSchedulerTest, BestEffortCompensationIsDisabledByDefault) {
  ...
  EXPECT_EQ(quick_done_future.wait_for(std::chrono::milliseconds(120)), std::future_status::timeout);
...
TEST(TaskSchedulerTest, CompensationWorkersReturnToZeroAfterIdle) {
  ...
  ASSERT_EQ(quick_done_future.wait_for(std::chrono::milliseconds(400)), std::future_status::ready);
...
TEST(TaskSchedulerTest, BestEffortIsolationRemainsStableUnderRounds) {
  ...
  for (int round = 0; round < kRounds; ++round) {
    ...
    ASSERT_EQ(normal_gate_started_future.wait_for(std::chrono::milliseconds(300)), std::future_status::ready)
        << "round=" << round;
    ...
    thread_pool.PostTaskWithTraits(
        FROM_HERE,
        nei::TaskTraits::UserBlocking(),
        nei::BindOnce([](std::promise<void> &done) { done.set_value(); }, std::ref(user_blocking_done)));

    EXPECT_EQ(best_effort_done_future.wait_for(std::chrono::milliseconds(300)), std::future_status::ready)
        << "round=" << round;

    EXPECT_EQ(user_blocking_done_future.wait_for(std::chrono::milliseconds(30)), std::future_status::timeout)
        << "round=" << round;

    release_normal_gate.set_value();
    EXPECT_EQ(user_blocking_done_future.wait_for(std::chrono::seconds(1)), std::future_status::ready)
        << "round=" << round;
  }
}
...
TEST(TaskSchedulerTest, MayBlockCompensationRemainsStableUnderRounds) {
  ...
  for (int round = 0; round < kRounds; ++round) {
    ...
    EXPECT_EQ(quick_done_future.wait_for(std::chrono::milliseconds(400)), std::future_status::ready)
        << "round=" << round;

    EXPECT_EQ(blocker_done_future.wait_for(std::chrono::milliseconds(30)), std::future_status::timeout)
        << "round=" << round;

    release_blocker.set_value();
    EXPECT_EQ(blocker_done_future.wait_for(std::chrono::seconds(1)), std::future_status::ready) << "round=" << round;
  }
}
...
TEST(TaskSchedulerTest, ConcurrentPostTaskAndStartShutdownStress) {
  ...
  thread_pool.PostTaskWithTraits(
      ...
      nei::TaskTraits::UserBlocking().WithShutdownBehavior(nei::ShutdownBehavior::BLOCK_SHUTDOWN),
      nei::BindOnce(
          [](std::promise<void> &started, std::shared_future<void> release, std::atomic<int> &executed) {
            started.set_value();
            release.wait();
  ...
  ASSERT_EQ(blocker_started_future.wait_for(std::chrono::milliseconds(500)), std::future_status::ready);
    ...
    posters.emplace_back([&]() {
      ...
      for (int i = 0; i < kPostsPerThread; ++i) {
        ...
        const nei::TaskTraits traits =
            mark_block_shutdown
                ? nei::TaskTraits::UserVisible().WithShutdownBehavior(nei::ShutdownBehavior::BLOCK_SHUTDOWN)
                : nei::TaskTraits::UserVisible().WithShutdownBehavior(nei::ShutdownBehavior::SKIP_ON_SHUTDOWN);
        ...
        thread_pool.PostTaskWithTraits(
            FROM_HERE,
            traits,
            nei::BindOnce([](std::atomic<int> &executed) { executed.fetch_add(1, std::memory_order_acq_rel); },
                          std::ref(executed_tasks)));
      }
  ...
  std::thread shutdown_thread([&]() { thread_pool.StartShutdown(); });
  ...
  std::future<void> shutdown_future = std::async(std::launch::async, [&]() { thread_pool.Shutdown(); });
  ...
  EXPECT_LE(executed_tasks.load(std::memory_order_acquire), attempted_posts.load(std::memory_order_acquire) + 1);
}
