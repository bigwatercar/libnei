...
public:
  ...
  nei::OnceCallback<void()> ScheduleAreaRecompute() {
...
TEST(CancelableCallbackTest, GeometricCalculatorDestructionAutoCancelsQueuedTask) {
  ...
  nei::OnceCallback<void()> queued_task;
...
TEST(TaskCallbackTest, OnceCallbackRunsAtMostOnce) {
  int count = 0;
  nei::OnceCallback<void()> cb = nei::BindOnce([&](int delta) { count += delta; }, 3);
...
TEST(TaskCallbackTest, OnceCallbackSupportsMoveOnlyArguments) {
  int out = 0;
  nei::OnceCallback<void()> cb = nei::BindOnce(
...
TEST(TaskCallbackTest, RepeatingCallbackRunsMultipleTimes) {
  int count = 0;
  nei::RepeatingCallback<void()> cb = nei::BindRepeating([&](int delta) { count += delta; }, 2);
...
TEST(TaskCallbackTest, RepeatingCallbackIsCopyable) {
  ...
  nei::RepeatingCallback<void()> copied = cb;
...
TEST(TaskCallbackTest, BindOnceCanBindMemberFunction) {
  ...
  Counter counter;
  nei::OnceCallback<void()> cb = nei::BindOnce(&Counter::Add, &counter, 5);
...
TEST(TaskCallbackTest, BindOnceCanBindThisPointer) {
  ...
  public:
    nei::OnceCallback<void()> MakeAddCallback(int delta) {
  ...
  nei::OnceCallback<void()> cb = counter.MakeAddCallback(7);
