...
std::filesystem::path MakeTempFilePath(const char *name_hint) {
  ...
  p /= std::string("nei_async_file_win_") + name_hint + "_" + std::to_string(GetCurrentProcessId()) + "_"
...
bool IssueRead(const std::shared_ptr<AsyncFileWin> &file,
               std::uint64_t offset,
...
public:
  FileBackedAsyncInputStream(std::shared_ptr<AsyncFileWin> file, std::size_t chunk_size)
      : file_(std::move(file))
  ...
  void ScheduleNextRead() {
    ...
    const bool accepted =
        IssueRead(file_,
                  offset_,
                  read_size,
                  [this, buf](bool success, std::vector<std::uint8_t> &&data, AsyncFile::Error /*error*/) mutable {
...
TEST(AsyncFileWinTest, OpenMissingFileReturnsGenericNoSuchFile) {
  ...
  io_runner->PostTask(FROM_HERE, [&, io_runner, bg_runner]() {
    ...
    file->OpenAsync(PathToUTF8(path),
  ...
  EXPECT_EQ(error_code.load(std::memory_order_acquire), AsyncFile::ErrorCode::kNotFound);
...
TEST(AsyncFileWinTest, LargeReadWriteCallbackDeterminismOnIoThread) {
  ...
  io_runner->PostTask(FROM_HERE, [&, io_runner, bg_runner, path_utf8, payload]() mutable {
    ...
    file->OpenAsync(path_utf8,
                    ...
                    bg_runner,
                    [&, file, payload, open_called](bool open_success, AsyncFile::Error open_error) mutable {
                      open_called->store(true, std::memory_order_release);
                      if (PlatformThread::CurrentId() != io_tid.load(std::memory_order_acquire)) {
  ...
  const AsyncFileWin::StageCounters counters = AsyncFileWin::GetStageCountersForTesting();
  const MessagePumpForIO::DebugCounters pump_counters = MessagePumpForIO::GetDebugCountersForTesting();
  const TaskRunnerTracingStats task_counters = TaskRunner::GetTracingStatsForTesting();
  if (!completed) {
    ADD_FAILURE() << "stage counters: open=" << counters.open_reached << " write=" << counters.write_reached
                  << " read=" << counters.read_reached << " iocp_completed=" << counters.iocp_completed
                  << " context_hit=" << counters.context_hit << " context_miss=" << counters.context_miss
                  << " read_finalize_attempted=" << counters.read_finalize_attempted
                  << " read_posted=" << counters.read_posted
                  << " callback_weak_dropped=" << counters.callback_weak_dropped
                  << " callback_post_failed=" << counters.callback_post_failed
                  << " write_post_seq=" << counters.write_post_seq << " write_exec_seq=" << counters.write_exec_seq
                  << " read_post_seq=" << counters.read_post_seq << " read_exec_seq=" << counters.read_exec_seq
                  << " pump_do_work_calls=" << pump_counters.do_work_calls
                  << " pump_do_work_consumed=" << pump_counters.do_work_consumed
                  << " pump_wake_dispatches=" << pump_counters.wake_dispatches
                  << " tasks_posted=" << task_counters.posted_tasks << " tasks_started=" << task_counters.started_tasks
    ...
    file->OpenAsync(
        ...
        [&, file](bool open_success, AsyncFile::Error open_error) mutable {
          ...
          *run_round = [&, file, run_round](int round) mutable {
            ...
            const std::vector<std::uint8_t> expected = MakePayload(kBytesPerRound, static_cast<std::uint8_t>(round));
            ...
            const bool write_accepted = IssueWrite(
                ...
                0,
                expected,
                [&, file, run_round, round, expected, write_called](
                    bool write_success, std::size_t bytes_written, AsyncFile::Error write_error) mutable {
                  write_called->store(true, std::memory_order_release);
                  if (PlatformThread::CurrentId() != io_tid.load(std::memory_order_acquire)) {
                    callback_on_io_thread.store(false, std::memory_order_release);
                  }
                  if (!write_success || !write_error.ok() || bytes_written != expected.size()) {
                    ok.store(false, std::memory_order_release);
                    file->CloseAsync([&]() { done.Signal(); });
                    return;
                  }
                  ...
                  const bool read_accepted = IssueRead(
                      ...
                      0,
                      expected.size(),
                      [&, file, run_round, round, expected, read_called](
                          bool read_success, std::vector<std::uint8_t> &&data, AsyncFile::Error read_error) mutable {
                        read_called->store(true, std::memory_order_release);
                        if (PlatformThread::CurrentId() != io_tid.load(std::memory_order_acquire)) {
                          callback_on_io_thread.store(false, std::memory_order_release);
                        }
                        if (!read_success || !read_error.ok() || data != expected) {
                          ok.store(false, std::memory_order_release);
                          file->CloseAsync([&]() { done.Signal(); });
                          return;
                        }
...
TEST(AsyncFileWinTest, AppendModeAppendsIgnoringCallerOffsetInOrder) {
  ...
  const bool write1_accepted =
      IssueWrite(file, 0, payload1, [&](bool success, std::size_t bytes_written, AsyncFile::Error error) {
        write1_ok.store(success && error.ok(), std::memory_order_release);
  ...
  const bool write2_accepted =
      IssueWrite(file, 0, payload2, [&](bool success, std::size_t bytes_written, AsyncFile::Error error) {
        write2_ok.store(success && error.ok(), std::memory_order_release);
  ...
  reader->OpenAsync(path_utf8,
                    ...
                    bg_runner,
                    [&](bool success, AsyncFile::Error error) {
                      read_open_ok.store(success && error.ok(), std::memory_order_release);
  ...
  const bool read_accepted =
      IssueRead(reader, 0, total_size, [&](bool success, std::vector<std::uint8_t> &&data, AsyncFile::Error error) {
  ...
  ASSERT_TRUE(
      std::equal(payload2.begin(), payload2.end(), all_data.begin() + static_cast<std::ptrdiff_t>(payload1.size())));

  reader->CloseAsync([&]() { read_close_barrier.Signal(); });
...
TEST(AsyncFileWinTest, ConcurrentAppendPreservesWholeWriteBlocks) {
  ...
  for (int i = 0; i < kWriters; ++i) {
    payloads[static_cast<std::size_t>(i)].assign(kBlockSize, static_cast<std::uint8_t>('A' + i));
  }
  ...
  for (int w = 0; w < kWriters; ++w) {
    writers.emplace_back([&, w]() {
      for (int r = 0; r < kRoundsPerWriter; ++r) {
        const bool accepted = IssueWrite(file,
                                         0,
                                         payloads[static_cast<std::size_t>(w)],
  ...
  const std::size_t expected_total = static_cast<std::size_t>(kWriters * kRoundsPerWriter) * kBlockSize;
  const bool read_accepted =
      IssueRead(reader, 0, expected_total, [&](bool success, std::vector<std::uint8_t> &&data, AsyncFile::Error error) {
        read_ok.store(success && error.ok(), std::memory_order_release);
        all_data = std::move(data);
        read_done.Signal();
      });
...
TEST(AsyncFileWinTest, FileReadParsesLinesWithAsyncLineReader) {
  ...
  const bool write_accepted =
      IssueWrite(writer, 0, bytes, [&](bool success, std::size_t wrote, AsyncFile::Error error) {
        write_ok.store(success && error.ok() && wrote == bytes.size(), std::memory_order_release);
        write_done.Signal();
      });
  ...
  writer->CloseAsync([&]() { write_close_barrier.Signal(); });
  ...
  reader_file->OpenAsync(path_utf8,
                         AsyncFile::OpenMode::kReadOnly,
                         AsyncFile::OpenDisposition::kOpenExisting,
                         bg_runner,
                         [&](bool success, AsyncFile::Error error) {
  ...
  input_stream.Close();
  reader_file->CloseAsync([&]() { read_close_barrier.Signal(); });
  ASSERT_TRUE(read_close_barrier.TimedWait(std::chrono::seconds(10)));
...
TEST(AsyncFileWinTest, RaiiDestructionDoesNotHangAndDataIsFlushed) {
  ...
  // Scope block: create, open, write, then release  --  RAII destruction.
  {
    ...
    const bool accepted = IssueWrite(file, 0, payload, [&](bool success, std::size_t wrote, AsyncFile::Error error) {
      write_ok.store(success && error.ok() && wrote == payload.size(), std::memory_order_release);
      write_done.Signal();
    });
  ...
  reader->ReadAsync(rbuf, payload.size(), 0, [&](bool success, std::size_t n, AsyncFile::Error error) {
    read_ok.store(success && error.ok() && n == payload.size(), std::memory_order_release);
    read_done.Signal();
  });
  ASSERT_TRUE(read_done.TimedWait(std::chrono::seconds(10)));
  ASSERT_TRUE(read_ok.load(std::memory_order_acquire));
