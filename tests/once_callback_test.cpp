...
TEST(OnceCallbackTest, LambdaWithStringParam) {
  std::string captured;
  OnceCallback<void(std::string)> cb = [&captured](std::string s) { captured = std::move(s); };
...
TEST(OnceCallbackTest, LambdaWithConstRefParam) {
  std::string received;
  OnceCallback<void(const std::string &)> cb = [&received](const std::string &s) { received = s; };
...
TEST(OnceCallbackTest, LambdaWithMoveOnlyParam) {
  ...
  OnceCallback<void(std::unique_ptr<int>)> cb = [&received](std::unique_ptr<int> p) { received = std::move(p); };
...
TEST(OnceCallbackTest, LambdaWithMultipleParams) {
  int sum = 0;
  std::string concat;
  OnceCallback<void(int, const std::string &)> cb = [&sum, &concat](int x, const std::string &s) {
    sum = x;
    concat = s;
  };
...
TEST(OnceCallbackTest, DestroyWithoutRunning) {
  int destroyed = 0;
  {
    ...
    OnceCallback<void()> cb = [g = std::move(guard), &destroyed]() { destroyed = *g; };
    EXPECT_TRUE(cb);
