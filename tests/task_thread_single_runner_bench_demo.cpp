...
BenchmarkResult RunSumBenchmark(
    const std::string& label,
    nei::TaskRunner& runner,
...
public:
    ...
    static void EnqueueThunk(
        ...
        static_cast<BenchSingleThreadQueue*>(context)->Enqueue(
            from_here,
            traits,
            std::move(task),
            delay);
...
private:
    ...
    struct QueuedTaskCompare {
        bool operator()(const std::shared_ptr<QueuedTask>& lhs,
                        const std::shared_ptr<QueuedTask>& rhs) const {
    ...
    std::priority_queue<
        std::shared_ptr<QueuedTask>,
        std::vector<std::shared_ptr<QueuedTask>>,
        QueuedTaskCompare>
        queue_;
...
void PrintResult(const BenchmarkResult& result, std::uint32_t task_count, bool& pass) {
    ...
    std::cout << result.label
              ...
              << " | enqueue_ns_per_task=" << enqueue_ns_per_task
              << " | drain_ns_per_task=" << drain_ns_per_task
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Thread / SingleThreadTaskRunner benchmark: " << task_count
              << " tasks\n";
    ...
    const BenchmarkResult thread_sum =
        RunSumBenchmark("thread_sum", *thread_runner, task_count);
    const BenchmarkResult thread_noop =
        RunNoopBenchmark("thread_noop", *thread_runner, task_count);

    thread_runner_owner.Shutdown();

    BenchSingleThreadQueue queue;
    auto single_runner = std::make_shared<nei::SingleThreadTaskRunner>(
        nei::SingleThreadTaskRunner::EnqueueDelegate{
            &queue,
            &BenchSingleThreadQueue::EnqueueThunk,
        });

    const BenchmarkResult single_sum =
        RunSumBenchmark("single_thread_task_runner_sum", *single_runner, task_count);
    const BenchmarkResult single_noop =
        RunNoopBenchmark("single_thread_task_runner_noop", *single_runner, task_count);
