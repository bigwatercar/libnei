...
TEST_F(ScopedBlockingCallShutdownTest, BlockShutdownWithBlockingCalls) {
  ...
  for (int i = 0; i < 10; ++i) {
    env_.thread_pool().PostTaskWithTraits(
        FROM_HERE, TaskTraits(TaskPriority::USER_VISIBLE, ShutdownBehavior::BLOCK_SHUTDOWN), [&]() {
          ScopedBlockingCall blocking;
...
TEST_F(ScopedBlockingCallShutdownTest, ShutdownPrunesNonBlockTasksButKeepsBlockTasks) {
  ...
  ASSERT_EQ(blocker_started_future_1.wait_for(std::chrono::milliseconds(300)), std::future_status::ready);
  ASSERT_EQ(blocker_started_future_2.wait_for(std::chrono::milliseconds(300)), std::future_status::ready);
  ...
  for (int i = 0; i < kQueuedTasks; ++i) {
    ...
    thread_pool.PostTaskWithTraits(FROM_HERE, TaskTraits(TaskPriority::USER_VISIBLE, behavior), [&, behavior]() {
      ScopedBlockingCall blocking;
...
TEST_F(ScopedBlockingCallShutdownTest, ConcurrentPostingDuringShutdownDropsNonBlockTasks) {
  ...
  for (int i = 0; i < 100; ++i) {
    thread_pool.PostTaskWithTraits(
        FROM_HERE, TaskTraits(TaskPriority::USER_VISIBLE, ShutdownBehavior::SKIP_ON_SHUTDOWN), [&]() {
          ScopedBlockingCall blocking;
